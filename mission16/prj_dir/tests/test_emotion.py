...
class TestSpeak:
    ...
    @patch('pvv_mcp_server.mod_speak.pvv_mcp_server.mod_avatar_manager.set_anime_type')
    ...
    def test_speak_normal_case(self, mock_requests_post, mock_sf_read, mock_output_stream, mock_set_anime_type):
        ...
        # アニメーションキーの設定確認
        assert mock_set_anime_type.call_count == 2
        mock_set_anime_type.assert_any_call(style_id, "口パク")
        mock_set_anime_type.assert_any_call(style_id, "立ち絵")
    ...
    @patch('pvv_mcp_server.mod_speak.sf.read')
    @patch('pvv_mcp_server.mod_speak.requests.post')
    def test_speak_with_custom_parameters(self, mock_requests_post, mock_sf_read, mock_output_stream, mock_set_anime_type):
        """正常系: カスタムパラメータでの音声合成"""
        # Arrange
        style_id = 2
        msg = "テスト"
    ...
    @patch('pvv_mcp_server.mod_speak.pvv_mcp_server.mod_avatar_manager.set_anime_type')
    @patch('pvv_mcp_server.mod_speak.requests.post')
    def test_speak_audio_query_error(self, mock_requests_post, mock_set_anime_type):
        ...
        # アニメーションキーは設定されない
        mock_set_anime_type.assert_not_called()

    @patch('pvv_mcp_server.mod_speak.pvv_mcp_server.mod_avatar_manager.set_anime_type')
    @patch('pvv_mcp_server.mod_speak.sd.OutputStream')
    @patch('pvv_mcp_server.mod_speak.sf.read')
    @patch('pvv_mcp_server.mod_speak.requests.post')
    def test_speak_synthesis_error(self, mock_requests_post, mock_sf_read, mock_output_stream, mock_set_anime_type):
    ...
    @patch('pvv_mcp_server.mod_speak.sf.read')
    @patch('pvv_mcp_server.mod_speak.requests.post')
    def test_speak_audio_playback_error(self, mock_requests_post, mock_sf_read, mock_output_stream, mock_set_anime_type):
        """異常系: 音声再生エラー"""
        # Arrange
        style_id = 1
        msg = "再生エラーテスト"
    ...
    @patch('pvv_mcp_server.mod_speak.sf.read')
    @patch('pvv_mcp_server.mod_speak.requests.post')
    def test_speak_stream_write_error(self, mock_requests_post, mock_sf_read, mock_output_stream, mock_set_anime_type):
        """異常系: ストリーム書き込みエラー"""
        # Arrange
        style_id = 1
        msg = "書き込みエラーテスト"
    ...
    @patch('pvv_mcp_server.mod_speak.sf.read')
    @patch('pvv_mcp_server.mod_speak.requests.post')
    def test_speak_empty_message(self, mock_requests_post, mock_sf_read, mock_output_stream, mock_set_anime_type):
        """正常系: 空のメッセージでの呼び出し"""
        # Arrange
        style_id = 1
        msg = ""
